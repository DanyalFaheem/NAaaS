...
func getKeywords(w http.ResponseWriter, r *http.Request) {
	...
	if req.Location == "" {
		if len(req.Topics) > 0 {
			rows, err = db.Query(`   SELECT n.header, n.focus_time::date, n.category, n.link, n.topics, n.location_type,
			CASE 
		...
		if len(req.Topics) > 0 {
			query := fmt.Sprintf(`   SELECT n.header, n.focus_time::date, n.category, n.link, n.topics, n.location_type,
			CASE 
			WHEN n.location_type = 'Province' THEN p.name 
			WHEN n.location_type = 'District' THEN d.name
			WHEN n.location_type = 'Tehsil' THEN t.name 
	...
	var news []newsData
	for rows.Next() {
		err := rows.Scan(&header, &focus_time, &category, &link, &topics, &location_type, &focus_location, &coordinates)
		if err != nil {
			log.Fatal(err)
		}
		var temp newsData
...
func getLocation(w http.ResponseWriter, r *http.Request) {
	...
	if req.Location == "" {
		if len(req.Topics) > 0 {
			rows, err = db.Query(`   SELECT n.header, n.focus_time::date, n.category, n.link, n.topics, n.location_type, n.picture,
			CASE 
			WHEN n.location_type = 'Province' THEN p.name 
			WHEN n.location_type = 'District' THEN d.name
			WHEN n.location_type = 'Tehsil' THEN t.name 
		...
		err = db.QueryRow("select location_type from locations where name= $1", req.Location).Scan(&location_type)
		if len(req.Topics) > 0 {
			query := fmt.Sprintf(`   SELECT n.header, n.focus_time::date, n.category, n.link, n.topics, n.location_type, n.picture,
			CASE 
			WHEN n.location_type = 'Province' THEN p.name 
			WHEN n.location_type = 'District' THEN d.name
			WHEN n.location_type = 'Tehsil' THEN t.name 
	...
	for rows.Next() {
		err := rows.Scan(&header, &focus_time, &category, &link, &topics, &location_type, &picture, &focus_location, &coordinates)
		if err != nil {
			log.Fatal(err)
		}
